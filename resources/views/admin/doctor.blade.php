<x-app-layout>
    <div class="py-12">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="mb-4 flex justify-between">
                        <h1 class="text-2xl font-bold mb-4">Doctors Management</h1>
                        <button id="openAddUserModal"
                            class="bg-sky-500/100 hover:bg-sky-500/50 text-white px-4 py-2 rounded">Add Doctor</button>
                    </div>
                                    ...
                                    @foreach ($doctors as $doctor)
                                        <tr class="hover:bg-gray-100 transition-colors">
                                            ...
                                            <td class="px-4 py-4 text-sm text-gray-900 align-middle">
                                                <span class="font-semibold">{{ $doctor->nip }}</span>
                                            </td>
                                            <td class="px-4 py-4 text-sm text-gray-900 align-middle">
                                                {{ $doctor->nik ?? '-' }}
                                            </td>
                                            ...
                                            <td class="px-4 py-4 text-sm text-gray-900 align-middle">
                                                <span
                                                    ...
                                                    {{ $roles->firstWhere('id', $doctor->role_id)->name ?? 'Unknown' }}
                                                </span>
                                            </td>
                                            <td class="px-4 py-4 text-sm text-gray-900 align-middle">
                                                {{ $doctor->email }}
                                            ...
                                            <td class="px-4 py-4 text-sm text-gray-900 align-middle max-w-xs break-words truncate"
                                                style="max-width: 200px;" title="{{ $doctor->address ?? '-' }}">
                                                {{ Str::limit($doctor->address ?? '-', 40) }}
                                            </td>
                                            <td class="px-4 py-4 text-sm font-medium align-middle space-x-2">
                                                <button data-target="edit-user-{{ $doctor->id }}"
                                ...
                                @foreach ($doctors as $doctor)
                                    <div id="edit-user-{{ $doctor->id }}"
                                        ...
                                        <div class="bg-white p-6 rounded shadow-lg w-full max-w-md mx-4 sm:mx-0">
                                            <h2 class="text-lg font-bold mb-4 text-center">Edit Doctor</h2>
                                            <form action="{{ route('users.update', $doctor->id) }}" method="POST"
                                                enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
                                                <div class="mb-4 flex flex-col items-center">
                                                    <img src="{{ $doctor->photo ? asset('storage/' . $doctor->photo) : asset('images/default-profile.png') }}"
                                                        alt="Profile Photo"
                                                        class="w-20 h-20 rounded-full object-cover border border-gray-300 shadow mb-2">
                                                ...
                                                <div class="grid grid-cols-1 gap-3">
                                                        ...
                                                        <select id="role" name="role_id"
                                                            ...
                                                            @foreach ($roles as $role)
                                                                <option value="{{ $role->id }}"
                                                                    {{ $doctor->role_id == $role->id ? 'selected' : '' }}>
                                                                    {{ $role->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <div class="mb-2">
                                                        ...
                                                        <textarea id="address" name="address"
                                                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-1.5 text-xs focus:ring focus:ring-blue-200"
                                                            rows="2">{{ $doctor->address }}</textarea>
                                                    </div>
                                                </div>
        ...
        <div id="add-user" class="fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-20 hidden">
            <div class="bg-white p-6 rounded shadow-lg w-full max-w-md mx-4 sm:mx-0">
                <h2 class="text-lg font-bold mb-4 text-center">Add New Doctor</h2>
