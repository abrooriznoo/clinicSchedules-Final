<x-app-layout>
    ...
    <div class="py-12">
                                    ...
                                    @foreach ($admins as $admin)
                                        <tr class="hover:bg-gray-100 transition-colors">
                                            ...
                                            <td class="px-4 py-4 text-sm text-gray-900 align-middle">
                                                <span class="font-semibold">{{ $admin->nip }}</span>
                                            </td>
                                            <td class="px-4 py-4 text-sm text-gray-900 align-middle">
                                                {{ Str::limit($admin->nik ?? '-', 16, '') }}
                                            </td>
                                            <td class="px-4 py-4 text-sm text-gray-900 align-middle">
                                                {{ $admin->name }}
                                            </td>
                                            <td class="px-4 py-4 text-sm text-gray-900 align-middle">
                                                <span
                                                    ...
                                                    {{ $roles->firstWhere('id', $admin->role_id)->name ?? 'Unknown' }}
                                                </span>
                                            </td>
                                            <td class="px-4 py-4 text-sm text-gray-900 align-middle">
                                                {{ $admin->email }}
                                            ...
                                            <td class="px-4 py-4 text-sm font-medium align-middle space-x-2">
                                                <button data-target="edit-user-{{ $admin->id }}"
                                ...
                                @foreach ($admins as $admin)
                                    <div id="edit-user-{{ $admin->id }}"
                                        ...
                                        <div class="bg-white p-6 rounded shadow-lg w-full max-w-md mx-4 sm:mx-0">
                                            <h2 class="text-lg font-bold mb-4 text-center">Edit admin</h2>
                                            <form action="{{ route('users.update', $admin->id) }}" method="POST"
                                                enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
                                                <div class="mb-4 flex flex-col items-center">
                                                    <img src="{{ $admin->photo ? asset('storage/' . $admin->photo) : asset('images/default-profile.png') }}"
                                                        alt="Profile Photo"
        ...
        <div id="add-user" class="fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-20 hidden">
            <div class="bg-white p-6 rounded shadow-lg w-full max-w-md mx-4 sm:mx-0">
                <h2 class="text-lg font-bold mb-4 text-center">Add New admin</h2>
